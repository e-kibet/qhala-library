...
public class UserController {
	...
	public HashMap<String, Object> registerUser(@Valid @RequestBody  User user) {
		...
		if(data.isPresent()) {
			...
			Optional<User> emailOptional =userService.checkIfEmailIsTaken(user.getEmail_address());
			if(emailOptional.isPresent()) {
				...
				userService.registerUser(user.getFirst_name(),user.getLast_name(),user.getEmail_address(),user.getMsisdn(),user.getPassword()); 
